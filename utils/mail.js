...
const TEST_MAIL = 'libby.gibson81@ethereal.email';
...
export async function signUpEmail(newUser) {
  ...
  try {
    ...
    logger.error(`Error al enviar el e-mail de registro. ${error}`);
  }
}
...
export async function checkOutEMail(newOrder) {
  ...
  try {
    ...
    logger.error(`Error al enviar el e-mail de pedido. ${error}`);
  }
}
